  ...
  return (
    <>
      {tableDetails.map((detail) => (
        <TableItem key={detail.item_id} detail={detail}>
          {" "}
        </TableItem>
      ))}
    </>
...
const QuarterReport = () => {
  ...
  const [revenue, setRevenue] = useState({first_quart_rev:[], second_quart_rev:[], third_quart_rev:[], fourth_quart_rev:[]});
  ...
  const onSubmit = (e) => {
    ...
    Axios.post("http://localhost:5000/orders/generatequaterreport", {
      year: year,
    }).then((Response) => {
    ...
    Axios.post("http://localhost:5000/orders/generatesalesrevenuereport", {
      year: year,
    }).then((Response) => {
  ...
  var revenueTotal = (array)=> {
    var total = 0;
  
    for (var i = 0; i < array.length; i++) {
        total += array[i].revenue;
    }
    return total;
  };
    ...
    data: [
      {
        ...
        dataPoints: [
          { label: 'Q1', y: revenueTotal(revenue.first_quart_rev) },
          { label: 'Q2', y: revenueTotal(revenue.second_quart_rev) },
          { label: 'Q3', y: revenueTotal(revenue.third_quart_rev) },
          { label: 'Q4', y: revenueTotal(revenue.fourth_quart_rev) },
        ],
      },
    ],
  };
  ...
  return (
    <Box pt="100px">
      <Grid templateColumns="repeat(2, 1fr)">
        <Box width="auto" h="auto" overflow="hidden" pb="5px"></Box>
        ...
        <GridItem colSpan={2} m="5">
          <Table variant="simple">
            <TableCaption>
              Sales reports of specific products of {year} Business Year{" "}
